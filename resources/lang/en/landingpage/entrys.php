<?php

return [
    'expostition' => 'Exposição',
    'expostition_text' => 'É um projeto de cinema, arte e educação que propôe uma reflexão acerca do racismo estrutural através da investigação de leis que contribuiram para a inferiorização do povo negro e para as políticas de branqueamento racial da sociedade brasileira no final do século XIX e início do século XX..',
    'research' => 'Pesquisa',
    'research_text' => 'Barreira social criada em face de costumes, leis, diferenças econômicas para separar brancos de não brancos.',
    'education' => 'Educação',
    'education_text' => 'Barreira social criada em face de costumes, leis, diferenças econômicas para separar brancos de não brancos.',
    'movie' => 'Filme',
    'movie_text' => 'Barreira social criada em face de costumes, leis, diferenças econômicas para separar brancos de não brancos.',
];